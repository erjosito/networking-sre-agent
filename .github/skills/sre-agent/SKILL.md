...
| SRE agent deploy spins ~20-45 min then fails `The resource provision operation did not complete within the allowed timeout period` — deployment op stays `Running` but its `statusMessage` shows `Tenant <id> is not allowed to create agent identities` | The tenant is **not allowlisted for the SRE Agent (`Microsoft.App/agents`) preview** — it can't create the required first-party agent identity. Not transient; retries hit the same wall. | Deploy the rest of the lab with `-DeploySreAgent $false` (networking + on-prem extension + telemetry are unaffected). Get the tenant onboarded/allowlisted for the SRE Agent preview before deploying the agent. Check the real cause via `az deployment operation group list ... --query "[?...=='Microsoft.App/agents'].properties.statusMessage"`. |
